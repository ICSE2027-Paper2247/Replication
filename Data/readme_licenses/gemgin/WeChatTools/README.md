...
## 2024-01-15 微信域名检测接口升级
- 微信域名检测接口升级

## 2025-12-07 微信域名检测服务接口本地部署

最近微信经常掉线，而且使用人数较多，经常超时，先编译本地部署exe，单个本地部署接口，间隔至少8s请求。

微信域名检测服务接口本地部署，支持蓝白提示类检测，需自备windows服务器2h4g和微信。本接口一经售出，无法退换。.